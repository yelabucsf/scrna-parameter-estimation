...
def generate_datasets():
    ...
    for trial in range(50):
        all_sampled_inds.append(np.random.choice(inds, numinds, replace=False))
    print(all_sampled_inds)

    # For now, we'll stick to comparing CD14 vs CD4 cells
    for numcells in [500]:

        for trial, sampled_inds in enumerate(all_sampled_inds):

            print('sampled these inds', sampled_inds)
            ...
            for ind in sampled_inds:
                for ct in ['T4', 'cM']:
                    ...
                    if ind_ct_adata.shape[0] > numcells: #subsample
