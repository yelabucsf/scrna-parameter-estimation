...
def run_onek_sampled(pop, inds, trial):
    
    data_path  = '/s3data/lupus/'
    pos = pd.read_csv(data_path + 'mateqtl_input/sampled_pos/{}_{}_{}.tsv'.format(pop, inds, trial), sep='\t', index_col=0)
    cov = pd.read_csv(data_path + 'mateqtl_input/sampled_cov/{}_{}_{}.tsv'.format(pop, inds, trial), sep='\t', index_col=0).T
    ...
    for ct in cts:
        ...
        gene_snp_pairs = onek_replication.query('cg_cov == "{}"'.format(ct)).drop_duplicates(['gene', 'rsid'])
    ...
    #     adata.write(data_path + 'memento_1k/{}.h5ad'.format(ct))
        ...
        memento_result.to_csv(data_path + 'memento_1k/{}_{}_{}_{}.csv'.format(pop, ct, inds, trial), index=False)
        ...
        pseudobulk.T.to_csv(data_path + 'mateqtl_input/sampled_pseudobulk/{}_{}_{}_{}.csv'.format(pop, ct, inds, trial), sep='\t')
...
#         adata.obs['memento_size_factor'] = 1.0
        ...
        #     adata.write(data_path + 'memento_1k/{}.h5ad'.format(ct))
            memento_result = memento.get_1d_ht_result(adata)
            if parameter=='mean':
                memento_result.to_csv(data_path + 'memento_full/{}/{}_{}_block_{}_replicated.csv'.format(window,pop, ct, block), index=False)
            if parameter=='variability':
                memento_result.to_csv(data_path + 'memento_full/{}/{}_{}_block_{}_variability_no_approx_replicated.csv'.format(window,pop, ct, block), index=False)
