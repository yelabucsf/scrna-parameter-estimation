...
def run_sc_methods():
    ...
    with open('guide_pairs.pkl', 'rb') as f:
        guides = pkl.load(f)
    ...
    for target, guide_pair in guides.items():
        ...
        for guide in guide_pair:
            ...
            adata = sc.read(data_path + 'tfko140.single_cell.{}.{}.h5ad'.format(target, guide))
            ...
            conditions = adata.obs.WT.drop_duplicates().tolist()
            ...
            data1 = ttest_adata[ttest_adata.obs['WT'] =='T'].X.todense()
            data2 = ttest_adata[ttest_adata.obs['WT'] =='F'].X.todense()
            ...
            ttest_result = pd.DataFrame(
                ...
                columns=columns)
            ttest_result.to_csv(data_path + 'tfko140.sc.ttest.{}.{}.csv'.format(target, guide))
            ...
            mwu_result = pd.DataFrame(
                ...
                index=ttest_adata.var.index,
                columns=columns)
            mwu_result.to_csv(data_path + 'tfko140.sc.mwu.{}.{}.csv'.format(target, guide))

            # GLM approach
            glm_adata = adata.copy()
            dispersions = pd.read_csv(data_path + 'tfko140.dispersions.{}.{}.csv'.format(target, guide), index_col=0)
            ...
            for ind in inds:
                for condition in conditions:
                    ...
                    data = glm_adata[(glm_adata.obs['donor']==ind) & (glm_adata.obs['WT']==condition)].X.toarray()
                    sf = glm_adata[(glm_adata.obs['donor']==ind) & (glm_adata.obs['WT']==condition)].obs['memento_size_factor'].values
                    q = glm_adata[(glm_adata.obs['donor']==ind) & (glm_adata.obs['WT']==condition)].obs['q'].values
                    ...
                    meta.append((ind, int(condition=='F')))
            ...
            meta = pd.DataFrame(meta, columns=['ind', 'condition'])
            ...
            design = dmatrix('condition+ind', meta)
            ...
            weighted_mean_glm_results.to_csv(data_path + 'tfko140.sc.weighted_mean_glm.{}.{}.csv'.format(target, guide))
